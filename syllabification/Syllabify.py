...
class Syllabifier:
    ...
    def __init__(self, num_epochs = 30, num_layers = 3,
                 num_dims = 50, load = False,
    ...
    def build_model(self):
        ...
        * Model parameters should be set in the general constructor, mainly:
            - `num_epochs` (int) = maximum # epochs to train the model
            - `num_layers` (int) = # number of stacked LSTM
            - `num_dims` (int) = dimensionality of the LSTMs
        ...
        for i in range(self.num_layers):
            ...
            curr_enc_out = Bidirectional(LSTM(output_dim=self.num_dims,
            ...
            l2r = LSTM(output_dim=self.num_dims,
                       return_sequences=True,
                       activation='tanh',
                       name='left_enc_lstm_'+str(i + 1))(curr_input)
            r2l = LSTM(output_dim=self.num_dims,
                       return_sequences=True,
                       activation='tanh',
                       go_backwards=True,
    ...
    def fit(self):
        """
        * Fits the model during x `num_epochs`.
    ...
    def vectorize(self, data):
        ...
        * Returns np.arrays:
            - `X` the token representation of shape
              (num_words, max_token_len + 2, num_dims)
            - `Y` the label representation of shape
              (num_words, max_token_len + 2, 3)
        """
        tokens, segmentations = [], []
        for token in data:
            chars, labels = [], []
